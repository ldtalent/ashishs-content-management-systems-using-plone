...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t collective/training_demo-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t collective/training_demo-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name training_demo-backend-acceptance -d collective/training_demo-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name training_demo-frontend-acceptance --link training_demo-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/training_demo-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop training_demo-frontend-acceptance
	@docker stop training_demo-backend-acceptance
